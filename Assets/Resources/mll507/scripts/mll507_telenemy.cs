...
public class mll507_telenemy : BasicAICreature {
	...
	void OnCollisionEnter2D(Collision2D collision) {
		...
		if (otherTile != null && otherTile.hasTag(tagsWeChase)) {
			if (room.GetComponent<mll507_room>().sands.Count > 0)
			{
				foreach (Tile tile in otherTile.transform.parent.GetComponentsInChildren<Tile>())
				{
					if (tile.hasTag(TileTags.Dirt) || tile.tileName == "hole")
					{
						Vector3 tilePos = tile.transform.position;
						if (tile.hasTag(TileTags.Dirt))
						{
							tile.takeDamage(this, tile.health);
							Tile new_hole = spawnTile(hole_prefab, room.transform, room.GetComponent<Room>().roomGridX, room.GetComponent<Room>().roomGridY);
							new_hole.sprite.sortingLayerID = SortingLayer.NameToID("Below Floor");
							new_hole.transform.SetPositionAndRotation(tilePos, Quaternion.identity);
						} else if (tile.tileName == "hole")
						{
							tile.takeDamage(this, tile.health);
							Tile new_sand = spawnTile(sand_prefab, room.transform, room.GetComponent<Room>().roomGridX, room.GetComponent<Room>().roomGridY);
							new_sand.sprite.sortingLayerID = SortingLayer.NameToID("Below Floor");
							new_sand.transform.SetPositionAndRotation(tilePos, Quaternion.identity);
						}
					}
				}			
			}
