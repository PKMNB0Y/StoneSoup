...
public class mll507_shovel : Tile {
	...
	public GameObject sand_gren_prefab;
	...
	void OnTriggerEnter2D(Collider2D other) {
		if (_swinging && other.gameObject.GetComponent<Tile>() != null) {
			...
			if (otherTile != _tileHoldingUs && otherTile.hasTag(TileTags.Dirt))
			{
			...
			} else if (otherTile != _tileHoldingUs && !otherTile.hasTag(TileTags.CanBeHeld)) {
